...
def main() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        server.start()
        try:
            response = wait_for_https_response(settings, tls.root_certificate)
            validate_response(response)
        finally:
            server.terminate()
            server.join(timeout=5)
    print("https-loopback-probe: PASS")
...
def wait_for_https_response(settings: CompanionSettings, root_certificate: Path) -> bytes:
...
def request_taskpane(settings: CompanionSettings, context: ssl.SSLContext) -> bytes:
    ...
    with (
        ...
        return receive_response(secured)
